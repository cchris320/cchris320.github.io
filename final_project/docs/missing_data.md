...
- `images/1.png` 到 `images/34.png` 已接入 `muscle_relax.html`。
...
腹部與側腹已先補文字方法與可信來源，但尚未生成正式示範圖。網頁目前會顯示「示範圖待補」。

| key | 部位 | 目前狀態 | 建議補法 |
|-----|------|----------|----------|
| `abdomen.hands` | 腹式呼吸放鬆 | 文字已補，`gif: ''` | 補一張仰躺或坐姿腹式呼吸示範圖 |
| `obliques.hands` | 溫和側彎伸展 | 文字已補，`gif: ''` | 補一張坐姿或站姿側彎伸展示範圖 |

這兩個方法刻意不加入腹部滾筒、腹部按摩球或深層按壓，避免腹部器官、肋骨邊緣與不明腹痛風險。
...
| key | 動作 | 目前狀態 | 建議補法 |
|-----|------|----------|----------|
| `biceps_curl` | 二頭彎舉 | `gif: ''`，目前也未在卡片中渲染示範媒體 | 補靜態圖或短影片 |
| `triceps_extension` | 三頭肌伸展 | `gif: ''`，目前也未在卡片中渲染示範媒體 | 補靜態圖或短影片 |
| `shoulder_press` | 肩推 | `gif: ''`，目前也未在卡片中渲染示範媒體 | 補靜態圖或短影片 |
...
1. 補 `abdomen.hands`、`obliques.hands` 兩張放鬆示範圖。
2. 補健身模式三個動作的示範素材。
3. 補健身模式參考來源與 form check 設計說明。
4. 若要擴充放鬆模式，再從已移出的 8 筆開始查可信來源。
5. 其他「器材 × 肌群」組合等核心展示穩定後再補。
